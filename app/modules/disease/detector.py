...
class LeafDiseaseDetector:
    ...
    def analyze_image_features(self, img: Image.Image) -> Dict[str, float]:
        ...
        # Healthy green condition: G > R and G > B
        green_mask = (g > r * 1.05) & (g > b * 1.05)
        # Chlorosis (yellowing): R > 140, G > 140, B < 110
        yellow_mask = (r > 130) & (g > 130) & (b < 110)
        # Necrotic spots (brown/black): R < 90, G < 90, B < 90 or dark brown
    ...
    def detect(self, image_bytes: bytes, crop_hint: str = "", rain_forecast: bool = False) -> Dict[str, Any]:
        try:
            pil_img = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        except Exception:
            return {
                "success": False,
                "error": "अमान्य छवि फ़ाइल। कृपया पत्ते की स्पष्ट फोटो अपलोड करें।"
            }
        ...
        # Agronomic deduction heuristics aligned with AgriGPT-VL / vision models
        scores = {}
        for key in self.disease_keys:
            scores[key] = 0.05
            
        # Target matching based on visual cues and crop hint
        if "tomato" in crop_hint_lower or "टमाटर" in crop_hint_lower:
            ...
            # Automatic image feature deduction when no crop hint provided
